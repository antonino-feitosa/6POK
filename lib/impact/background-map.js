...
ig.BackgroundMap = ig.Map.extend({	
	...
	drawTiled: function() {	
		var tile = 0,
			...
			tileOffsetY = (this.scroll.y / (this.tilesize* 3/4)).toInt(),
			pxOffsetX = this.scroll.x % this.tilesize,
			pxOffsetY = this.scroll.y % (this.tilesize* 3/4),
			pxMinX = -pxOffsetX - this.tilesize,
			pxMinY = -pxOffsetY - this.tilesize * 3/4,
			pxMaxX = ig.system.width + this.tilesize - pxOffsetX,
			pxMaxY = ig.system.height + this.tilesize * 3/4 - pxOffsetY;
		...
		let ycount = 0;
		for( var mapY = -1, pxY = pxMinY; pxY < pxMaxY; mapY++, pxY += (this.tilesize * 3/4)) {
			...
			let incx = 0;
			if(tileY % 2 == 0){
				incx += this.tilesize/2;
			}
			
			for( var mapX = -1, pxX = pxMinX + incx; pxX < pxMaxX; mapX++, pxX += this.tilesize ) {
