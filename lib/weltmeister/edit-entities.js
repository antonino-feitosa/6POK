...
wm.EditEntities = ig.Class.extend({
	...
	moveSelectedEntity: function( x, y ) {
		let dx = 0;
		if(Math.floor(y/(this.gridSize*3/4)) % 2 == 0 ){
			dx = this.gridSize/2;
		}
		x = 
			Math.round( (x - this.selector.offset.x ) / this.gridSize )
			* this.gridSize + this.selectedEntity.offset.x + dx;
		y = 
			Math.round( (y - this.selector.offset.y ) / (this.gridSize*3/4) )
			* (this.gridSize*3/4) + this.selectedEntity.offset.y;
