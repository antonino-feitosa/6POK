...
ig.module(
    ...
    .defines(function () {
        ...
        HexGame = ig.Game.extend({
            ...
            _lookAtGridCone: function(gridpos, units, direction, checked){ // TODO
                if(units > 0){
                    let ents = this._lookAtGridPosition(gridpos);
                    for(let d of direction.cone){
                        ents += _lookAtGridCone(d.nextPosition(gridpos), units-1, d, checked).filter(e => this._filterId(e, checked));
                    }
                    return ents.filter(e => this._filterId(e, checked));
                }
                return [];
            },
            ...
            _lookAtGridRadius: function(gridpos, units, checked){ // TODO
                if(units > 0){
                    let ents = this._lookAtGridPosition(gridpos);
