...
ig.module(
    ...
    .defines(function () {
        ...
        SchedulingHeap = ig.Class.extend({
            ...
            forEach: function(call){
                this.heap.forEach(x => call(x.value));
            },
        ...
        HexGame = ig.Game.extend({
            ...
            updateEntities: function () {
                ...
                if (!this.firingTriggers.isEmpty()) {
                    this.firingTriggers.forEach(t => t.doTriggerLeave());
                    this.firingTriggers.forEach(t => t.doTriggerStay());
                    this.firingTriggers.forEach(t => t.doTriggerEnter());
                    this.firingTriggers.forEach(t => t.doTriggerInteract());

                    let running = new SchedulingHeap();
                    this.firingTriggers.forEach(t => t.doTrigger() ? running.push(t) : null);
            ...
            _lookAtGridPosition: function(gridpos){
                let ents = this.triggerList.filter(t => {
                    gpos = this.grid.coords2Grid(t.pos);
                    return gpos.x === gridpos.x && gpos.y === gridpos.y;
                });
                if(this.grid.isGridWall(gridpos)){
                    ents.push({isObstacle: true});
                } else {
            ...
            lookAtLine: function(startPos, units, direction){
                let gridpos = this.grid.coords2Grid(startPos);
                let ents = this._lookAtGridPosition(gridpos);
                for(let i=0;i<units;i++){
                    gridpos = direction.nextPosition(gridpos);
                    ents += this._lookAtGridPosition(gridpos);
                }
                return ents;
            },

            _lookAtGridCone: function(gridpos, units, direction, checked){ 
                if(units > 0 && !checked.has(gridpos.x * 10000 + gridpos.y)){ // TODO map width < 10000
                    let ents = this._lookAtGridPosition(gridpos);
                    checked.add(gridpos.x * 10000 + gridpos.y);
                    for(let d of direction.cone){
                        ents += _lookAtGridCone(d.nextPosition(gridpos), units-1, d, checked);
                    }
                    return ents;
                }
                return [];
            ...
            lookAtCone: function(startPos, units, direction){
                let gridpos = this.grid.coords2Grid(startPos);
                return this._lookAtGridCone(gridpos, units, direction, new Set());
            },

            _lookAtGridRadius: function(gridpos, units, checked){
                if(units > 0 && !checked.has(gridpos.x * 10000 + gridpos.y)){ // TODO map width < 10000
                    let ents = this._lookAtGridPosition(gridpos);
                    checked.add(gridpos.x * 10000 + gridpos.y);
            ...
            lookAtRadius: function(startPos, units){
                let gridpos = this.grid.coords2Grid(startPos);
                return this._lookAtGridRadius(gridpos, units, new Set());
            }
        });
    });
