...
ig.module('game.AnimalStats')
    .requires(
        ...
        Nature.randomize = function () {
            ...
            let pairs = [];
            let noHP = STAT.filter(st => st !== STAT.hp);
            noHP.forEach(first => noHP.forEach(second => pairs.push([first, second])));
            rand.shuffle(pairs);
            Nature.values = NATURE.map((name, index) => new Nature(name, pairs[index][0], pairs[index][1]));
        ...
        Animal = ig.Class.extend({
            ...
            init: function (level = 1) {
                this.name = ig.game.rand.pick(ANIMAL);
                this.level = level;

                rand = new Random(this.name + ig.global.gameSeed);
                this.type1 = rand.pick(TYPE);
                this.type2 = rand.pick(TYPE);
                this.nature = rand.pick(Nature.values);
                this.bv = STAT.map(_ => rand.nextInt(256));
                this.iv = STAT.map(_ => ig.game.rand.nextInt(32));
                this.ev = STAT.map(_ => 0);
                STAT.forEach(att => this[att] = () => this._calcStatistic(att));
                this.health = this.hp();
                ...
                this.yield = STAT.map(_ => rand.nextInt(4));
                let sum = 0;
                ...
                this.moves = this.moveList.popStartMoves();
            },

            onLearnMove: function (move) {
                this.moves.shift();
            ...
            earnExp: function (points) {
                ...
                while (points + this.exp > pointsNextLevel) {
                    ...
                    while (this.moveList.hasMove() && this.moveList.peekMove().minLevel <= this.level) {
                        let move = this.moveList.popMove();
                        this.onLearnMove(move);
                    }
            ...
            _calcStatistic: function (att) {
                ...
                st = att === STAT.hp ? st + this.level + 10 : Math.floor((st + 5) * nat);
                return st;
            }
        });
    });
