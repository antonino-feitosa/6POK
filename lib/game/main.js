    ...
    .defines(function () {
        ...
        MyGame = HexGame.extend({
            ...
            init: function () {
                ...
                //console.log(JSON.stringify({data:ig.global.playerAnimal}, null, 2));
            ...
            spawnEntity: function (type, x, y, settings) {
                ...
                if (ent.idGroup) {
                    ...
                    if (!this.map.groups[ent.idGroup]) {
                        this.map.groups[ent.idGroup] = [];
                    }
                    let group = this.map.groups[ent.idGroup];
                    group.push(ent);
                }
                return ent;
            },
            ...
            removeEntity: function (ent) {
                ...
                if (ent.idGroup) {
                    let group = this.map.groups[ent.idGroup];
                    this.map.groups[ent.idGroup] = group.filter(e => e.id != ent.id);
                }
            },

            getGroup(entity) {
                if (entity.idGroup && this.map.groups) {
                    let group = this.map.groups[entity.idGroup];
                    return group;
                }
                return [];
            },
            ...
            writeData: function () {
                fetch('gamedata.js', {
                    ...
                    body: JSON.stringify({ data: ig.global.playerAnimal }, null, 2)
                });
            },
