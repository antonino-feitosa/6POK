...
ig.module(
    ...
    .defines(function () {
        ...
        Move = ig.Class.extend({
            ...
            _processTargetType(look) {
                ...
                if (this.target & TARGET.ENEMY) {
                    ...
                    targets.all.concat(targets.enemies);
                }
        ...
        EffectPound = {
            ...
            makeStart() {
                let options = [
                    { name: 'Punch I ', power: 50, charge: 8, level: 1, accuracy: 1 },
                    { name: 'Pound I ', power: 40, charge: 10, level: 1, accuracy: 1 },
                    { name: 'Scratch I ', power: 40, charge: 10, level: 1, accuracy: 1 },
                    { name: 'Anger Pound I ', power: 60, charge: 8, level: 1, accuracy: 0.8 }
                ];
            ...
            makeLeveling() {
                let options = [
                    { name: "Punch II ", power: 75, charge: 6, level: 12, accuracy: 1 },
                    { name: "Punch III ", power: 90, charge: 4, level: 24, accuracy: 1 },
                    { name: "Punch IV ", power: 115, charge: 2, level: 36, accuracy: 1 },
                    { name: "Scratch II ", power: 60, charge: 8, level: 6 },
                    { name: "Scratch III ", power: 80, charge: 6, level: 12 },
                    { name: "Scratch IV ", power: 100, charge: 4, level: 18 },
                    ...
                    { name: "Anger Pound II ", power: 80, charge: 6, level: 12, accuracy: 0.8 },
                    { name: "Anger Pound III ", power: 100, charge: 4, level: 24, accuracy: 0.8 },
                    { name: "Anger Pound IV ", power: 120, charge: 2, level: 36, accuracy: 0.8 }
                ];
                return this._fillOptions(options);
            },
            ...
            makeFinal() {
                let options = [
                    { name: 'Punch V ', power: 130, charge: 1, level: 48, accuracy: 1 },
                    { name: 'Scratch V ', power: 100, charge: 4, level: 24, accuracy: 1 },
                    { name: 'Pound V ', power: 120, charge: 2, level: 32, accuracy: 1 },
                    { name: 'Anger Pound V ', power: 150, charge: 1, level: 48, accuracy: 0.8 }
                ];
                return this._fillOptions(options);
            }
        };
        ...
        MoveList = ig.Class.extend({
            ...
            init(animal) {
                if (!MoveList.lists[animal.name]) {
                    let start = MoveList.startMoves.filter(move => move.type === animal.type1 || move.type === animal.type2);
                    let end = MoveList.finalMoves.filter(move => move.type === animal.type1 || move.type === animal.type2);
                    let leveling = MoveList.levelingMoves.filter(move => move.type === animal.type1 || move.type === animal.type2);
                    let others = MoveList.levelingMoves.filter(move => move.type !== animal.type1 && move.type !== animal.type2);

                    let rand = new Random(animal.name);
                    let list = [];
                    list.concat(rand.sample(start, 4));
                    list.concat(rand.sample(leveling, 12));
                    list.concat(rand.sample(others, 6));
                    list.concat(rand.sample(end, 8));
